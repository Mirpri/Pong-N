...
p_name=['balanced','large','friction','swift','Sparky','Lightning','Archer' ,'Shadow','(test)','<random>']
p_width=[40,          55,      30,       25,     30,        35    ,   38    ,   30   , 1000,      30]
p_fric=[100,         100,      40,       200,    100,      300    ,   110   ,   120     , 100,       100]
p_speed=[13,          10,       11,      18,     10,        12    ,   14    ,   15   ,  0,         0]
p_color=['gray','tan','darkseagreen','blueviolet','peru','skyblue' ,'orchid', 'black','white','white']
p_ac=[    0,          0,        0,      0,        0.4,      0.5,      1 ,      0.7  ,  0,       0]
p_ex=[   200,         200,      200,     200,     100,       80,       50,     10    ,  200,     200]
p_cnr=[     1.4,      1.2,      0.9,     1.5,     1.2,        1    ,    1   ,  1.4   ,    1,      1]
...
def Archer_hit(i):
    ...
    if i==1:
        p1ready=0        
        vx,vy=(p1a-x)/((p1a-x)**2+(400-y)**2)**0.5*20,(400-y)/((p1a-x)**2+(400-y)**2)**0.5*20
    if i==2:
        p2ready=0        
        vx,vy=(p2a-x)/((p2a-x)**2+(400-y)**2)**0.5*20,(400-y)/((p2a-x)**2+(400-y)**2)**0.5*20
def Shadow_app(i):
    global p1p,p2p,p1,p2,x
    print(p1,x,x-p1)
    if i==1:
        M.move(p1p,x-p1,0)
        p1=x
...
ball=M.create_oval(300-10,400-10,300+10,400+10,fill='red',width=0)
drawpad()
...
def match(ii=5):
    ...
    c2=C2.get()
    if c1=='<random>':
        c1=p_name[random.randint(0,3)]
        C1.set(c1)
    if c2=='<random>':
        c2=p_name[random.randint(0,3)]
        C2.set(c2)
    todrawpad(0)
